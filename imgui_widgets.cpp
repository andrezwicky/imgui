...
bool ImGui::Selectable_int(const char* label, int selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const int was_selected = selected;
    bool hovered, held;
    bool pressed = ButtonBehavior(bb, id, &hovered, &held, button_flags);
...
bool ImGui::Selectable_int(const char* label, int* p_selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    if (Selectable_int(label, *p_selected, flags, size_arg))
    {
        *p_selected = !*p_selected;
        return true;
    }
